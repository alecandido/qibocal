...
@dataclass
class TestClassificationParameters(Parameters):
...
@dataclass
class TestClassificationData(Data):
    nshots: int
    """Number of shots."""
    savedir: str
    """Dumping folder of the classification results"""
...
@dataclass
class TestClassificationResults(Results):
...
def _acquisition(
    params: TestClassificationParameters,
    platform: Platform,
    qubits: Qubits,
...
def _fit(data: TestClassificationData) -> TestClassificationResults:
    qubits = data.qubits
...
def _plot(
    data: TestClassificationData, qubit, fit: TestClassificationResults
):
    fitting_report = ""
...
def _update(
    results: TestClassificationResults, platform: Platform, qubit: QubitId
):
...
test_single_shot_classification = Routine(_acquisition, _fit, _plot, _update)
